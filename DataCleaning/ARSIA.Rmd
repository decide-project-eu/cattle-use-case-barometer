...
# DATACLEANING FOR ARSIA - Version 2025
...
barometer_dt_raw_2 <- readxl::read_excel("../Data/ARSIA/DECIDE_COMPIL_DATA__202501272012.xlsx")
...
barometer_dt_raw <- barometer_dt_raw_2
```
...
barometer_dt <- barometer_dt_raw %>% 
  ...
  tidyr::separate(ADRESS, c('Postal_code', 'City')) %>%
  dplyr::mutate(
  ...
  dplyr::distinct() %>%
  dplyr::mutate(
  # Robust date parsing: try ymd_hms (with tz/offset), then ymd, then dmy
    Date = dplyr::coalesce(
      suppressWarnings(lubridate::as_date(lubridate::ymd_hms(Date, quiet = TRUE))),
      suppressWarnings(lubridate::ymd(Date)),
      suppressWarnings(lubridate::dmy(Date))
    ),
    Date_month = lubridate::floor_date(Date, "month"),
    # Make weeks start Monday (Belgian convention); adjust if you prefer Sunday
    Date_week  = lubridate::floor_date(Date, "week", week_start = 1),
...
#barometer_dt$Date <- ymd(barometer_dt$Date)
