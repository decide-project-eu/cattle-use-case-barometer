...
## Aggregate data based on filenumber

```{r}
barometer_groupby <- barometer %>%
  group_by(Lab_reference, Country, Breed, Date, Postal_code, Province, Farm_ID, Filenumber, Diagnostic_test, 
           Sample_type, Pathogen) %>%
...
agg_barometer_monthly <- barometer_groupby %>% 
  group_by(Lab_reference, Country, Breed, Date, Postal_code, Province, Farm_ID, Filenumber, Diagnostic_test,
           Sample_type, Year, Month, Pathogen) %>%
  summarise(across(c(Result), max))
