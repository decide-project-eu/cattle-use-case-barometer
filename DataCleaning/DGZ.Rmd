...
barometer_aero_cult <- barometer_aero_cult_raw %>%
  ...
  dplyr::mutate(
    ...
    Samplenumber_anon = sha256_hash(as.character(Samplenumber))
  ) %>%
  dplyr::select(-Filenumber, -Samplenumber)
print(barometer_aero_cult)
...
# Print the barometer_myco_cult dataset
...
# Print the barometer_dt dataset
print(barometer_dt)
```
...
barometer <-
  ...
  dplyr::left_join(
      barometer_aero_cult, by = c('Filenumber_anon', 'Samplenumber_anon', 'Result', 'Parameter_code', 'Pathogen_identification')
      ) %>%
  dplyr::left_join(
      barometer_myco_cult, by = c('Filenumber_anon', 'Samplenumber_anon', 'Result', 'Parameter_code', 'Pathogen_identification')
  ) %>%
  dplyr::mutate( 
...
```{r}
write.csv(barometer_groupby, "../DataCleaning/Cleaned_data/barometer_DGZ.csv", row.names=TRUE)
...
>>>>>>> 04b09b352111dcce0038251a1a8862d3d8d7b889
```
